...
task runr, "Runs app for current platform":
 exec &"nim c -r {releaseOpts} -o:app src/main.nim"

task rund, "Runs debug app for current platform":
 exec &"nim c -r {debugOpts} -o:app src/main.nim"

task release, "Builds app for current platform":
 exec &"nim c {releaseOpts} -o:app src/main.nim"

task webd, "Builds debug app for web":
 exec &"nim c {debugOpts} -d:emscripten -o:app.html src/main.nim"

task webr, "Builds release app for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:app.html src/main.nim"

task debug, "Builds debug app for current platform":
 exec &"nim c {debugOpts} -o:app_debug src/main.nim"
